...
from slavoj.domain.interfaces import BookProcessorInterface, LLMInterface, \
    DatabaseInterface
...
class BookProcessor(BookProcessorInterface):
    ...
    async def process_query(self,
        ...
        try:
            ...
            tasks = [
                self._process_book_with_semaphore(book, conversation_context,
                                                  query, semaphore)
                for book in books
            ]

            responses = await asyncio.gather(*tasks)
            return [r for r in responses if
                    r is not None]  # Filter out any failed responses
    ...
    async def process_single_book(self,
        ...
        try:
            ...
            raise BookProcessingError(
                f"Failed to process book {book.title}: {e}")

    async def _process_book_with_semaphore(self,
                                           book: Book,
                                           conversation_context: ConversationContext,
                                           query: str,
                                           semaphore: asyncio.Semaphore) -> GeneratedResponse:
        ...
        async with semaphore:
            try:
                ...
                self.logger.error(
                    f"Processing timed out for book: {book.title}")
                return None
            except Exception as e:
